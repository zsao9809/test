<template>
    <div class="work">
        <div class="header">
            <el-page-header @back="goBack" :title="template.template_name" :icon="null">
                <template #content>
...
import { editTemplate, getTemplateInfo } from '@/http/api';
...
const update = () => {
    getReportInfo(template_id).then(res => {
        if (res.data.code != 1) {
            return ElMessage({
                message: '同步模板信息失败',
                type: 'error',
        ...
        form.value = template.value.template_data.list[index];
    })
}
const initTemplate = store.state.template_list.find(e => e.template_id == template_id);
if (initTemplate) {
    template.value = initTemplate;
    form.value = template.value.template_data.list[index];
} else update();
...
const onSubmit = async () => {
    ...
    template.value.template_data.list[index] = form.value;
